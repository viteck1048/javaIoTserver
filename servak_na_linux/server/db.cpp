...
//			char result[256];
			...
			puts("err create");
			isc_print_status(status_vector);
			// Сюди потрапляємо, лише коли база і не приєдналась, і не створилась --
			// тобто вона справді відвалилась. Далі кожен запит однаково впаде на битому
			// db_handle (поверненого значення connect_db() ніхто не перевіряє), а вбити
			// процес ззовні вже не вийде: fbclient перехопив сигнали.
			// connect_db() кличуть УСІ обробники, тож помирає перший же клік по базі,
			// а не тільки головна сторінка.
			// Код 1, а не 0: для systemd (Restart=on-failure) це збій, і сервер
			// підніметься сам. Чистий exit(0) лишив би його лежати.
			puts("--------------EXIT(1): DB LOST--------------");
			exit(1);
		}
